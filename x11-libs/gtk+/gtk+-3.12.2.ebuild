...
IUSE="aqua broadway cloudprint colord cups debug examples +introspection test vim-syntax wayland X xinerama"
REQUIRED_USE="
	|| ( aqua wayland X )
	xinerama? ( X )
"
...
pkg_postinst() {
	...
	if use broadway; then
		ewarn "The broadway backend is experimental!"
		ewarn "Use \`GDK_BACKEND=broadway ...\` to enable this."
	fi
}
